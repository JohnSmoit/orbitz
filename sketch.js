...
function draw() {
  ...
  for (let i = 0; i < planets.length; i++) {
    ...
    planets[i].display();
    stroke(255, 0, 0);
    line(planets[i].position.x, planets[i].position.y, planets[i].position.x + planets[i].velocity.x * 10, planets[i].position.y + planets[i].velocity.y * 10);
    //print(planets[i].velocity.x + ", " + planets[i].velocity.y);
  }
  fill(0);
}